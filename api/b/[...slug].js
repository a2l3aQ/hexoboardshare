...
function buildSVG(grid, dark) {
  ...
  const R = 30;
  const col = {
    empty:   dark ? '#1c1c1c' : '#d4d0c8', stroke:  dark ? '#2e2e2e' : '#b8b4ac',
  ...
  const sz = Math.max(3, R*0.22), lw = Math.max(1, sz*0.5), hid = 'ho';
  ...
  for (const { q, r, x, y } of coords) {
    ...
    const cx = x+ox, cy = y+oy, d = hexPath(cx, cy, R);
...
export default async function handler(req, res) {
  ...
  const boardCode = code.split('~')[0];
  const grid = decode(boardCode);
  ...
  const appHref = `${APP_URL}?b=${encodeURIComponent(code)}`;
...
<meta property="og:description" content="Open in HEX STRATEGY to view this position.">
